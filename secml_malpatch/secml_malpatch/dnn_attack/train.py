...
if __name__ == "__main__":
    ...
    chkpt_acc_path = 'finetuned_malconv.pth'
    ...
    dataloader = DataLoader(
        ExeDataset(list(tr_table.index), train_data_path, list(tr_table.ground_truth), 2**20),
        batch_size=32, shuffle=True, num_workers=8)
    validloader = DataLoader(
        ExeDataset(list(val_table.index), valid_data_path, list(val_table.ground_truth), 2**20),
    ...
    # net = DNN_Net(input_length=4096)
    net = MalConv()
    # net = CClassifierEnd2EndMalware(net)
    net.load_simplified_model('/home/omnisky/zhan/secml_malpatch/secml_malware/data/trained/pretrained_malconv.pth')

    bce_loss = nn.BCELoss()
    adam_optim = optim.Adam([{'params': net.parameters()}], lr=0.001)
    sigmoid = nn.Sigmoid()
    ...
    while total_step < 20000:
        ...
        for step, batch_data in enumerate(dataloader):
            ...
            exe_input = Variable(exe_input.float(), requires_grad=False)
            ...
            history['tr_acc'].extend(
                ...
                list(label.cpu().data.numpy().astype(int) == ((pred).cpu().data.numpy() + 0.5).astype(int)))
        ...
        if valid_best_acc < np.mean(history['val_acc']):
            ...
            torch.save(net.state_dict(), chkpt_acc_path)
